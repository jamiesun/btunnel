...
20 字节报头在 AEAD 之外，因此若以明文传输，即便没有特征字，*被动* 观察者也能凭恒定的 `version`、
重复的 `epoch`、低位单调的 `seq` 对协议做指纹识别。部署级的 `obfuscate` 开关（**默认开启**）以**零字节
开销**消除该指纹：发送方仅对报头用每包 pad 做 XOR 掩码

```text
pad = BLAKE2b-256(key = link_key, "subnetra-v1-obfs" || tag)[0..20]
```

pad 由方向性 `link_key` 与数据报自身的 16 字节明文 `tag` 派生；包体本就伪随机、保持不变。XOR 对称，
故接收方用公开的 tag 即可还原。由于被掩码的 `key_id` 无法再直接读取，入站会**逐一试**每个对端的接收
链路密钥（重算 pad、校验 `version` + `key_id`、再解掩），AEAD 认证仍是真正的安全闸。它**不协商**——
每个节点必须把 `obfuscate` 设成一致（不一致则 fail-closed）——且只隐藏协议指纹，**不**隐藏包长或时序。
开启混淆的 spoke 还会把 NAT 保活间隔在 `[keepalive_secs/2, keepalive_secs]` 内随机化，使保活*节奏*不构成
固定周期特征。**默认开启**；设 `obfuscate: false` 可关闭，关闭后线上字节与 v1 完全一致（报头在抓包中可读）。
完整规范与 KAT 向量见 [`docs/PROTOCOL.md` §3.4](https://github.com/jamiesun/subnetra/blob/main/docs/PROTOCOL.md)。
...
1. **身份选择**：按 `key_id`（而非端点）。无匹配对端 ⇒ 丢弃。*（开启[报头混淆](#报头混淆可选)后，
   报头被掩码，本步改为逐一试每个对端的接收链路密钥来解掩；第 2–9 步不变。）*
