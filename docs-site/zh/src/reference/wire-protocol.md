...
20 字节报头是明文且在 AEAD 之外，因此即便没有特征字，*被动* 观察者仍能凭恒定的 `version`、重复的
`epoch`、低位单调的 `seq` 对协议做指纹识别。可选的、部署级的 `obfuscate` 开关以**零字节开销**消除该
指纹：发送方仅对报头用每包 pad 做 XOR 掩码
...
默认关闭，线上字节与之前完全一致。完整规范与 KAT 向量见
[`docs/PROTOCOL.md` §3.4](https://github.com/jamiesun/subnetra/blob/main/docs/PROTOCOL.md)。
