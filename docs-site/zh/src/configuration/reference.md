...
| `listen_ports` | 整数数组 | `[18020, 18023, 18026]` | 守护进程为承载绑定的 UDP 端口——**显式列出**（绝非范围计算）。本节点在 **全部** 端口上接收数据报，并按每个对端最近一次到达的本地 socket 回程发送（NAT 正确），因此单个端口被封锁不会让本节点下线。1–8 个端口，每个非零且互不相同。默认值刻意 **不是** `51820`（WireGuard 的知名端口本身就是指纹）。 |
| `listen_port` | 整数 | _(未设置)_ | 单端口便捷/向后兼容写法：等价于 `"listen_ports": [<port>]`。当 `listen_ports` 存在时本字段被忽略；两者都省略时使用默认端口集。 |
| `virtual_subnet` | CIDR | `10.0.0.0/24` | 本网格构建的叠加子网。 |
| `local_tun_ip` | CIDR | _(不设)_ | 本节点自身的叠加地址（主机 + 前缀），例如 `10.0.0.2/24`。用于生成[主机网络规划](network-plan.md)、为 TUN 配置地址；守护进程 **不** 自行配置主机地址。`role=spoke` 需要一个本地目标（本字段 **或** `local_routes`），并以它作为该 Spoke 的可达 IP。对 `role=hub` 为 **可选**——推导出的表只转发给 Spoke，故不设它即让 Hub 保持纯中继、在叠加网络上 **不可达**；仅当需要访问 Hub 自身时才设置它（并追加一条本地投递规则）（[详情](roles.md#访问-hub-自身)）。 |
...
| `endpoint` | 字符串 | — | 对端承载地址，形如 `host:port`，例如 `203.0.113.2:18020`（使用对端 `listen_ports` 中的某个端口）。Hub 使用动态 DNS 时见[部署指南](../operations/deployment.md)。 |
