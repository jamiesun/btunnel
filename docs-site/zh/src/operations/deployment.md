...
有一条从固定公网 `IP:port` 到 Hub 内网 `IP:listen_port` 的**静态 1:1 端口映射（DNAT）**，那么
位于 NAT 之后的主机同样符合条件：

- **Spoke 拨向_外部_地址。** 每个 Spoke 的对端 `endpoint` 是该映射的公网 `IP:port`，而非
  Hub 的私网地址。
- **`listen_port` 是_内部_目标。** 把它设为 DNAT 投递到的端口（例如公网 `51820` → 内网
  `51820` ⇒ `listen_port = 51820`）。
