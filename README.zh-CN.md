...
- **默认加密，链路上低特征**——每个包都用 ChaCha20-Poly1305 加密，**每条链路独立密钥**，
  ...
  看来，这个隧道就像没有任何东西在监听。20 字节封装报头默认明文，因此*被动*观察者仍可对协议做
  指纹识别；开启**可选的报头混淆**（`"obfuscate": true`，需全 mesh 一致）即可掩盖它，使整个数据报
  看起来像随机字节——它隐藏的是协议指纹，而非包长或时序。见 [`docs/PROTOCOL.md` §3.4](docs/PROTOCOL.md)。
